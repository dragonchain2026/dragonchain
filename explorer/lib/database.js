...
module.exports = {
  ...
  get_distribution: function(richlist, stats, cb){
    ...
    var len = richlist.balance.length;
    lib.syncLoop(len, function (loop) {
      var i = loop.iteration();
      var count = i + 1;
      var bal = parseInt(richlist.balance[i] ? richlist.balance[i].balance : 0) || 0;
      var balDrgn = bal / 100000000;
      var percentage = stats.supply > 0 ? (balDrgn / stats.supply) * 100 : 0;
      if (count <= 25 ) {
        distribution.t_1_25.percent += percentage;
        distribution.t_1_25.total += balDrgn;
      } else if (count <= 50) {
        distribution.t_26_50.percent += percentage;
        distribution.t_26_50.total += balDrgn;
      } else if (count <= 75) {
        distribution.t_51_75.percent += percentage;
        distribution.t_51_75.total += balDrgn;
      } else if (count <= 100) {
        distribution.t_76_100.percent += percentage;
        distribution.t_76_100.total += balDrgn;
      }
      loop.next();
    }, function(){
      var trackedTotal = distribution.t_1_25.total + distribution.t_26_50.total + distribution.t_51_75.total + distribution.t_76_100.total;
      distribution.t_101plus.total = parseFloat((distribution.supply - trackedTotal).toFixed(8));
      distribution.t_101plus.percent = (100 - distribution.t_76_100.percent - distribution.t_51_75.percent - distribution.t_26_50.percent - distribution.t_1_25.percent).toFixed(2);
