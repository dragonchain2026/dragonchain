...
bool static IsCompressedOrUncompressedPubKey(const valtype &vchPubKey) {
    // Falcon-512 public key: 0x07 prefix + 897 bytes = 898 bytes total
    if (vchPubKey.size() == 898 && vchPubKey[0] == 0x07) {
        return true;
    }
    // Original secp256k1 checks (for backwards compatibility)
...
bool static IsCompressedPubKey(const valtype &vchPubKey) {
    // Falcon-512 public key: accept as "compressed" for segwit
    if (vchPubKey.size() == 898 && vchPubKey[0] == 0x07) {
        return true;
    }
    // Original secp256k1 checks
    if (vchPubKey.size() != 33) {
        //  Non-canonical public key: invalid length for compressed key
        return false;
    }
...
bool static IsValidSignatureEncoding(const std::vector<unsigned char> &sig) {
    ...
    // Minimum and maximum size constraints for secp256k1 DER signatures.
...
uint256 SignatureHash(const CScript& scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    ...
    if (sigversion == SIGVERSION_WITNESS_V0) {
        ...
        // Dragonchain FORKID: inject chain-specific prefix to prevent cross-chain replay
        if (nHashType & SIGHASH_FORKID) {
            CHashWriter ss2(SER_GETHASH, 0);
            ss2 << std::string("Dragonchain-ForkID-v1");
            ss2 << hash;
            return ss2.GetHash();
        }
