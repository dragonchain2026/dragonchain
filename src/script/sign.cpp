...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    // Test solution
    // Skip VerifyScript for DummySignatureCreator (it creates invalid signatures for fee calculation)
    const DummySignatureCreator* dummy_creator = dynamic_cast<const DummySignatureCreator*>(&creator);
    if (dummy_creator) {
        return solved;
    }
...
{
    // Create a dummy signature that matches Falcon-512 size for fee calculation
    // Falcon-512 signature is ~690 bytes + 1 sighash byte = 691 bytes
    vchSig.assign(691, '\000');
    vchSig[690] = SIGHASH_ALL;
    return true;
}
