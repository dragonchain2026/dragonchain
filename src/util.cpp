...
const char * const BITCOIN_CONF_FILENAME = "dragonchain.conf";
const char * const BITCOIN_PID_FILENAME = "dragonchaind.pid";
...
#else
    const char* pszModule = "dragonchain";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Dragonchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Dragonchain";
#else
    // Unix
    return pathRet / ".dragonchain";
#endif
#endif
}
