...
void CKey::DerivePubKey() {
    ...
    const unsigned char* sk = keydata.data();
    ...
    v = PQCLEAN_FALCON512_CLEAN_trim_i8_decode(
        ...
        sk + u, PRIVATE_KEY_SIZE - u);
    if (v == 0) { fValid = false; return; }
    u += v;
    v = PQCLEAN_FALCON512_CLEAN_trim_i8_decode(
    ...
    v = PQCLEAN_FALCON512_CLEAN_modq_encode(
        pubkeydata.data() + 1, PUB_KEY_SIZE - 1,
        h, 9);
    if (v != PUB_KEY_SIZE - 1) {
        fValid = false;
        return;
...
CPubKey CKey::GetPubKey() const {
    if (!fValid) {
        // Return an empty public key if not valid
        CPubKey empty;
        return empty;
    }
    CPubKey pubkey;
...
void CExtKey::Encode(unsigned char code[BIP32_EXTKEY_SIZE]) const {
    ...
    code[41] = 0;
    if (key.size() < 32)
        throw std::runtime_error("CExtKey::Encode: key size too small");
