...
#if defined(NDEBUG)
# error "Dragonchain cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Dragonchain Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // First 6 months: 10,500,000 coins (half of total 21,000,000)
    // Then every 4 years: subsidy halved

    const int64_t BLOCKS_PER_HALF_YEAR = 3153600;  // 182.5 days * 24 * 3600 / 5
    const int64_t BLOCKS_PER_FOUR_YEARS = 25228800; // 4 years * 365 * 24 * 3600 / 5

    CAmount nSubsidy;

    if (nHeight < BLOCKS_PER_HALF_YEAR) {
        // First half year: 10,500,000 coins total
        nSubsidy = 332988658;  // ~3.33 coins
    } else {
        // After first half year: halving every 4 years
        int64_t blocks_after_first = nHeight - BLOCKS_PER_HALF_YEAR;
        int halvings = blocks_after_first / BLOCKS_PER_FOUR_YEARS;

        if (halvings >= 64)
            return 0;

        // nSubsidy = 10,500,000 / (25,228,800 * 2) = 0.20807311 DRAGON/block
        nSubsidy = 20807311;  // ~0.208 coins per block
        nSubsidy >>= halvings;
    }
...
void ThreadScriptCheck() {
    RenameThread("dragonchain-scriptch");
