...
bool CPubKey::RecoverCompact(const uint256 &hash, const std::vector<unsigned char>& vchSig) {
    ...
    if (vchSig.size() < PUBLIC_KEY_SIZE - 1) {
        return false;
    }
...
bool CPubKey::IsFullyValid() const {
    if (!IsValid())
        return false;
    if (vch[0] == 0x07 && size() == PUBLIC_KEY_SIZE)
        return true;
...
/* static */ bool CPubKey::CheckLowS(const std::vector<unsigned char>& vchSig) {
    // Falcon-512 signature: LowS check doesn't apply, always return true
    // Falcon-512 signatures can start with any byte and have variable length
