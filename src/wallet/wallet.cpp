...
CPubKey CWallet::GenerateNewKey(CWalletDB &walletdb, bool internal)
{
    ...
    // Disable HD key derivation for Falcon-512 - always generate new key directly
...
void CWallet::ReserveKeyFromKeyPool(int64_t& nIndex, CKeyPool& keypool, bool fRequestedInternal)
{
    ...
    keypool.vchPubKey = CPubKey();
    {
        ...
        // Disable HD internal key pool for Falcon-512 - always use external pool
        bool fReturningInternal = false;
        std::set<int64_t>& setKeyPool = setExternalKeyPool;
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (fFirstRun)
    {
        ...
        // Disable HD wallet for Falcon-512 compatibility
        walletInstance->SetMinVersion(FEATURE_LATEST);

        // Generate a simple key instead of HD master key
        CKey key;
        key.MakeNewKey(true);
        CPubKey pubkey = key.GetPubKey();
        {
            LOCK(walletInstance->cs_wallet);
            if (!walletInstance->AddKeyPubKey(key, pubkey))
                throw std::runtime_error(std::string(__func__) + ": AddKeyPubKey failed");
        }
...
void CWallet::LearnRelatedScripts(const CPubKey& key, OutputType type)
{
    if (key.IsCompressed() && (type == OUTPUT_TYPE_P2SH_SEGWIT || type == OUTPUT_TYPE_BECH32)) {
        ...
        // Disable assert for Falcon-512 - IsSolvable may not work correctly
