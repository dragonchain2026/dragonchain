...
void Shutdown()
{
    ...
    RenameThread("dragonchain-shutoff");
...
// FIXME.SUGAR
// QT About Dragonchain
std::string LicenseInfo()
{
    const std::string URL_SOURCE_CODE = "<https://github.com/dragonchain2026/dragonchain>";
    const std::string URL_WEBSITE = "<https://dragonchain.cc>";
    ...
    return
           ...
           Copyright_5 + "\n" +
           Copyright_6 + "\n" +
           CopyrightHolders(strprintf(_("Copyright (C) %i"), 2026) + " ") + "\n" +
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("dragonchain-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if dragonchain is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if dragonchain is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        if (!fLoaded && !fRequestShutdown) {
            // Auto-recover from database corruption without user interaction.
            // 1st attempt: -reindex-chainstate (fast, rebuilds UTXO set only)
            // 2nd attempt: -reindex (full, rebuilds block index + UTXO set)
            // 3rd attempt: abort with error
            static int nAutoRecoverAttempts = 0;
            nAutoRecoverAttempts++;

            if (nAutoRecoverAttempts == 1 && !fReindexChainState) {
                LogPrintf("*** Auto-recovering database: -reindex-chainstate (attempt %d of 2)\n",
                          nAutoRecoverAttempts);
                fprintf(stderr, "Database corruption detected, auto-recovering with reindex-chainstate (attempt %d of 2)...\n",
                        nAutoRecoverAttempts);
                fReindexChainState = true;
                fRequestShutdown = false;
            } else if (nAutoRecoverAttempts <= 2) {
                LogPrintf("*** Auto-recovering database: -reindex (attempt %d of 2)\n",
                          nAutoRecoverAttempts);
                fprintf(stderr, "Database corruption detected, auto-recovering with full reindex (attempt %d of 2)...\n",
                        nAutoRecoverAttempts);
                fReindex = true;
                fRequestShutdown = false;
            } else {
                LogPrintf("*** Auto-recovery failed after %d attempts.\n", nAutoRecoverAttempts);
    ...
    // Wait for genesis block to be processed
    {
        ...
        // Note: Do not disconnect BlockNotifyGenesisWait to avoid boost::function casting issues with newer boost versions
