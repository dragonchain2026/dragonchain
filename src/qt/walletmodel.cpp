...
WalletModel::WalletModel(const PlatformStyle *platformStyle, CWallet *_wallet, OptionsModel *_optionsModel, QObject *parent) :
    ...
    cachedNumBlocks(0), m_last_balance_check(0)
{
        ...
        */
        ...
        if(chainActive.Height() - cachedNumBlocks >= 12)
        {
            ...
            */
            ...
            if (now_ms - m_last_balance_check >= 60000) {
                m_last_balance_check = now_ms;
                checkBalanceChanged();
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
...

    {
        TRY_LOCK(cs_main, lockMain);
        if (!lockMain) {
            Q_EMIT message(tr("Send Coins"), tr("Network is busy. Please try again in a moment."),
                         CClientUIInterface::MSG_WARNING);
            return SendCoinsReturn(TransactionCreationFailed);
        }
        TRY_LOCK(wallet->cs_wallet, lockWallet);
        if (!lockWallet) {
            Q_EMIT message(tr("Send Coins"), tr("Wallet is busy. Please try again in a moment."),
                         CClientUIInterface::MSG_WARNING);
            return SendCoinsReturn(TransactionCreationFailed);
        }
